...
static void toQuartonian(double& qw, double& qx, double& qy, double& qz, double roll, double pitch, double yaw)
{
...
static void hatmap(const Eigen::Vector3d &w, Eigen::Matrix3d &skew){
	skew.setZero();
	skew(1,0) = w(2);
	skew(0,1) = -w(2);
	skew(2,0) = -w(1);
...
static void veemap(const Eigen::Matrix3d &skew, Eigen::Vector3d &w){
	w(0) = skew(2,1);
	w(1) = skew(0,2);
	w(2) = skew(1,0);
}

static void skewmat(const Eigen::VectorXd &X, Eigen::MatrixXd &skewmatrix){
	skewmatrix.setZero();
