...
void KF::updateKalman(const int contactIndex[4], float IMU[3], const float quat[4], const double relVec[12]){
    ...
    // ====================== Update Q_ Matrix ====================== // 
    // ============================================================== // 
    double spdfl = (contactIndex[0]==2) ? 1e-1 : (contactIndex[0]==1) ? 1 : 5e1;
    double spdfr = (contactIndex[1]==2) ? 1e-1 : (contactIndex[1]==1) ? 1 : 5e1;
    double spdhl = (contactIndex[2]==2) ? 1e-1 : (contactIndex[2]==1) ? 1 : 5e1;
    double spdhr = (contactIndex[3]==2) ? 1e-1 : (contactIndex[3]==1) ? 1 : 5e1;
    ...
    Q_(3, 3) = SAX;  Q_(4, 4) = SAY;  Q_(5,5) = SAZ;  
    Q_(6, 6) = FL;   Q_(7, 7) = FL;   Q_(8, 8) = FL;  
    Q_(9, 9) = FR;   Q_(10, 10) = FR; Q_(11, 11) = FR;
    Q_(12, 12) = RL; Q_(13, 13) = RL; Q_(14, 14) = RL;
    Q_(15, 15) = RR; Q_(16, 16) = RR; Q_(17, 17) = RR;

    // ============================================================== //
    // ===================== Measurement Vector ===================== // 
    // ============================================================== //
    double toeOffset = 0.038;
    Eigen::Matrix<double, 16, 1> yk;
    yk << relVec[0], relVec[1], relVec[2],  relVec[3], 
          relVec[4], relVec[5], relVec[6],  relVec[7], 
          relVec[8], relVec[9], relVec[10], relVec[11], 
    ...
    S      = C_*Pk_*C_.transpose()+R_;
    K      = Pk_*C_.transpose()*S.inverse();
    x_temp = A_*xk_+B_*imu;
    xk_    = x_temp + K*(yk - C_*x_temp);
    Pk_    = (Eigen::Matrix<double, 18, 18>::Identity() - K*C_)*Pk_;
}
